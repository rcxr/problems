...
static struct __stream __atrapando_in, __atrapando_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__atrapando_in, "\\\\.\\pipe\\libinteractive_Main_atrapando_in", O_WRONLY);
	openstream(&__atrapando_out, "\\\\.\\pipe\\libinteractive_Main_atrapando_out", O_RDONLY);
}
...
static void __message_loop_atrapando(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__atrapando_out, &__msgid, sizeof(__msgid), 0)) {
		if (__msgid == __current_function) return;
		switch (__msgid) {
			case 0x62cd5a6u: {
				// atrapando -> Main.ponMuro
				uint32_t __cookie;
				int ponMuro_f;
				readfull(&__atrapando_out, &ponMuro_f, sizeof(int), 1);
				int ponMuro_c;
				readfull(&__atrapando_out, &ponMuro_c, sizeof(int), 1);
				int ponMuro_d;
				readfull(&__atrapando_out, &ponMuro_d, sizeof(int), 1);
				readfull(&__atrapando_out, &__cookie, sizeof(__cookie), 1);

				int result =
				ponMuro(ponMuro_f, ponMuro_c, ponMuro_d);

				writefull(&__atrapando_in, &__msgid, sizeof(__msgid));
				writefull(&__atrapando_in, &result, sizeof(result));
				writefull(&__atrapando_in, &__cookie, sizeof(__cookie));
				writeflush(&__atrapando_in);
				break;
			}
...
void atrapando(int n) {
	const uint32_t __msgid = 0x514c2134u;
	const uint32_t __cookie = 0x26b0e6a7u;

	writefull(&__atrapando_in, &__msgid, sizeof(__msgid));
	writefull(&__atrapando_in, &n, sizeof(int));
	writefull(&__atrapando_in, &__cookie, sizeof(__cookie));
	writeflush(&__atrapando_in);

	__message_loop_atrapando(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__atrapando_out, &__cookie_result, sizeof(__cookie_result), 1);
