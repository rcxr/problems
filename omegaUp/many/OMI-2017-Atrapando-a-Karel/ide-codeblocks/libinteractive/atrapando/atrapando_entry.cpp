...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x514c2134u: {
				// Main -> atrapando.atrapando
				uint32_t __cookie;
				int atrapando_n;
				readfull(&__in, &atrapando_n, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				atrapando(atrapando_n);
...
int ponMuro(int f, int c, int d) {
	const uint32_t __msgid = 0x62cd5a6u;
	const uint32_t __cookie = 0x26b0e6a7u;

	writefull(&__out, &__msgid, sizeof(__msgid));
	writefull(&__out, &f, sizeof(int));
	writefull(&__out, &c, sizeof(int));
	writefull(&__out, &d, sizeof(int));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "\\\\.\\pipe\\libinteractive_atrapando_atrapando_in", O_RDONLY);
	openstream(&__out, "\\\\.\\pipe\\libinteractive_atrapando_atrapando_out", O_WRONLY);
